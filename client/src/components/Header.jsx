...
const Header = () => {
    ...
    return (
        <>
            ...
            <div className='h-[60px] max-w-[1200px] mx-auto w-full flex justify-start gap-[30px] items-center z-[5] border-b-[0.3px] mb-[20px]'>
                <NavLink className={'text-lg text-gray-900 font-medium hover:text-green-600'} to={`/`}>Home</NavLink>
                {categories && categories ? categories?.map(category =>
                    <>
                        <NavLink className={'text-lg text-gray-900 font-medium hover:text-green-600'} to={`/blogs/category/${category._id}`}>{category?.name}</NavLink>
                    </>
                ) : null}
            </div>
        </>
